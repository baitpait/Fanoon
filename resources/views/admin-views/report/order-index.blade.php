    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-3">
            <div class="card-body">
                <form action="{{route('admin.report.set-date')}}" method="post">
                    @csrf
                    <div class="mb-2">
                        <label class="form-label">{{translate('Show data by date range')}}</label>
                    </div>
                    <div class="row align-items-end g-3 bg-light rounded p-3 mb-2">
                        ...
                        <div class="col-12 col-lg-4 d-flex gap-2 align-items-end">
                            <button type="submit" class="btn btn-primary flex-grow-1">{{translate('show')}}</button>
                            <a href="{{ route('admin.report.order') }}" class="btn btn-soft-secondary flex-grow-1 text-center">{{ translate('clear') }}</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $delivered=\App\Models\Order::notPos()->where(['order_status'=>'delivered'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $returned=\App\Models\Order::notPos()->where(['order_status'=>'returned'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $failed=\App\Models\Order::notPos()->where(['order_status'=>'failed'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $canceled=\App\Models\Order::notPos()->where(['order_status'=>'canceled'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
            ...
            @php
                $delivered=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $delivered[$i]=\App\Models\Order::notPos()->where(['order_status'=>'delivered'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $ret=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $ret[$i]=\App\Models\Order::notPos()->where(['order_status'=>'returned'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $fai=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $fai[$i]=\App\Models\Order::notPos()->where(['order_status'=>'failed'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $can=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $can[$i]=\App\Models\Order::notPos()->where(['order_status'=>'canceled'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
