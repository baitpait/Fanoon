    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-3">
            <div class="card-body">
                <form action="{{route('admin.report.set-date')}}" method="post">
                    @csrf
                    <div class="mb-2">
                        <label class="form-label">{{translate('show_data_by_data_range')}}</label>
                    </div>
                    <div class="row align-items-end g-3 bg-light rounded p-3 mb-2">
                        ...
                        <div class="col-12 col-lg-4 d-flex gap-2 align-items-end">
                            ...
                            <a href="{{ route('admin.report.earning') }}" class="btn btn-soft-secondary flex-grow-1 text-center">{{ translate('clear') }}</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="row">
            @php
                ...
                $to = session('to_date');
                $totalTax=\App\Models\Order::notPos()->where(['order_status'=>'delivered'])
            ...
            <div class="col-sm-6 mb-3">
                @php
                    $totalSold=\App\Models\Order::notPos()->where(['order_status'=>'delivered'])
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-3">
                @php
                    $yearlySold=\App\Models\Order::notPos()->where(['order_status'=>'delivered'])->whereBetween('created_at', [date('y-01-01'), date('y-12-31')])->sum('order_amount')
                @endphp
            ...
            @php
                $sold=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $sold[$i]=\App\Models\Order::notPos()->where(['order_status'=>'delivered'])->whereBetween('created_at', [$from, $to])->sum('order_amount');
                    }
            @endphp
            ...
            @php
                $tax=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $tax[$i]=\App\Models\Order::notPos()->where(['order_status'=>'delivered'])->whereBetween('created_at', [$from, $to])->sum('total_tax_amount');
                    }
            @endphp
